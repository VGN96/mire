...
export function startCronJobs() {
  ...
  cron.schedule('30 7 * * *', async () => {
    ...
    } catch (e) { console.error('[CRON] OOTD:', e.message); }
  ...
  cron.schedule('0 9 * * 0', async () => {
    try {
      ...
      const byUser = worn.reduce((a, i) => { (a[i.userId] = a[i.userId] || []).push(i.name); return a; }, {});
    ...
    } catch (e) { console.error('[CRON] Laundry:', e.message); }
  }, { timezone: 'Asia/Kolkata' });
